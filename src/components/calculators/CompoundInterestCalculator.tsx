...
export default function CompoundInterestCalculator() {
  ...
  const [compounding, setCompounding] = useState<1 | 2 | 4 | 12 | 365>(12);

  useEffect(() => {
    const detected = detectCurrency();
    setCurrency(detected);
    setPrincipal(detected.defaultLoan / 3);
  }, []);
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        <div className="lg:col-span-3 bg-white border border-gray-200 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6">
          ...
          <div>
            <label className="text-sm font-semibold text-gray-700 mb-2 block">Compounding Frequency</label>
            <div className="flex flex-wrap gap-2">
              {([{ v: 1, l: "Annually" }, { v: 2, l: "Semi-Annual" }, { v: 4, l: "Quarterly" }, { v: 12, l: "Monthly" }, { v: 365, l: "Daily" }] as const).map((opt) => (
                <button key={opt.v} type="button" onClick={() => setCompounding(opt.v)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold border-2 transition-all ${compounding === opt.v ? "border-amber-500 bg-amber-50 text-amber-700" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}>{opt.l}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
            <p className="text-sm font-semibold text-gray-700 mb-4">Growth Curve</p>
            <div className="h-48">
