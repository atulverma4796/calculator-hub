...
export const metadata: Metadata = {
  title: "About CalcHub",
  description: "CalcHub provides 15+ free online calculators for finance, math, health, and utility. No signup, no limits, beautiful results.",
...
export default function AboutPage() {
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
      <h1 className="text-3xl font-extrabold text-gray-900 mb-2">About CalcHub</h1>
      <p className="text-sm text-gray-500 mb-8">Free online calculators for everyone, everywhere.</p>

      <div className="prose prose-gray max-w-none space-y-6 text-gray-700 text-sm leading-relaxed">
        <section>
          <h2 className="text-lg font-bold text-gray-900 mt-8 mb-3">Our Mission</h2>
          <p>
        ...
        <section>
          <h2 className="text-lg font-bold text-gray-900 mt-8 mb-3">What Makes CalcHub Different</h2>
        ...
        <section>
          <h2 className="text-lg font-bold text-gray-900 mt-8 mb-3">Our Calculators</h2>
          <div className="space-y-4 mt-4">
            {CATEGORIES.map((cat) => (
              <div key={cat.id}>
                <h3 className="text-sm font-bold text-gray-800">{cat.icon} {cat.name}</h3>
                <div className="flex flex-wrap gap-2 mt-2">
                  {CALCULATOR_LIST.filter((c) => c.category === cat.id).map((c) => (
                    <Link
                      key={c.slug}
                      href={`/calculator/${c.slug}`}
                      className="px-3 py-1.5 bg-gray-100 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg text-xs font-medium text-gray-700 transition-colors"
                    >
                      {c.icon} {c.shortName}
                    </Link>
                  ))}
        ...
        <section>
          <h2 className="text-lg font-bold text-gray-900 mt-8 mb-3">Feedback</h2>
          <p>
